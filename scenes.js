            ...
            if (ctx.session.tableM3.rows[0] != undefined) { // если не пустая
                ...
                for (let i = ctx.session.tableM3.rows.length - 1; i >= 0; i--) {
                    ...
                    if (today <= dateUp && today > weekBefore) {
                        ...
                        ctx.scene.step = 2;
                        ctx.reply(`📅 В срок до ${dateUp.format('L')} дежурят по кухне - ${ctx.session.tableM3.rows[i]['Кухня']}, по КВТ - ${ctx.session.tableM3.rows[i]['КВТ']}. 
                        ...
                        ctx.reply(`🆓 На текущий период дежурств не запланировано. 
                                    ...
                                    🧹 Последний раз дежурили по кухне - ${ctx.session.tableM3.rows[ctx.session.tableM3.rows.length - 1]['Кухня']}, по КВТ - ${ctx.session.tableM3.rows[ctx.session.tableM3.rows.length - 1]['КВТ']}. 
